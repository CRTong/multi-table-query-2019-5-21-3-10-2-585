    ...
    SELECT * FROM 
	    (select
            ...
            sum(c.score) /  count(c.courseId) as avg
                FROM student_course c, student s 
                WHERE c.studentId = s.id 
                GROUP BY stu_id) temp 
	                WHERE temp.avg >= 60
...
# 8.查询每门课程的平均成绩，结果按平均成绩降序排列，平均成绩相同时，按课程编号升序排列
    SELECT  sc.courseId as courseId, avg(sc.score) AS avg
    FROM student_course sc 
    GROUP BY sc.courseId 
    ORDER BY avg DESC , sc.courseId ASC ;

# 9.查询课程名称为"数学"，且分数低于60的学生姓名和分数
    SELECT  s.*
    FROM student_course sc,student s 
    WHERE s.id = sc.studentId and sc.courseId = (
            SELECT c.id 
            FROM course c 
            WHERE c.name = '数学')
            and sc.score < 60
